...
app.config(function($provide, $urlRouterProvider, StateDefaultParams, GlobalConfig) {
  $provide.constant('ManagerAppStateConfigs', _.filter([
    {
      name: 'terms',
      url: '/terms',
...
app.config(function($provide, ManagerAppStateConfigs, StateDefaultParams) {
  // Auto-apply StateDefaultParams to all second level states
  _([ManagerAppStateConfigs]).flatten().each(function(state) {
