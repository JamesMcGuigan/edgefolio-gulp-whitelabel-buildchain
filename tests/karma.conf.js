...
function getKarmaConfig() {
  ...
  var includesJSON = {
    "pollyfills":  JSON.parse(fs.readFileSync(path.join(__dirname, '../src/production/edgefolio-pollyfills-includes.json')).toString()),
    "investorapp": JSON.parse(fs.readFileSync(path.join(__dirname, '../src/production/edgefolio-investorapp-includes.json')).toString()),
    "managerapp":  JSON.parse(fs.readFileSync(path.join(__dirname, '../src/production/edgefolio-managerapp-includes.json' )).toString())
  };
  ...
  includesFiles = _([])
    .concat('src/production/edgefolio-gulpConfig-investor-app.js')
    .concat(includesFiles)
    .flatten(true)
    .unique()
    .concat('tests/chai.conf.js')
    .concat('src/bower_components/angular-mocks/angular-mocks.js')
    .concat('src/bower_components/karma-read-json/karma-read-json.js')
    .concat({ pattern: 'tests/**/*.json', included: false }) // needed to get readJSON to work
    .concat('tests/unit-preload/**/*.js')
    .concat('tests/unit/**/*.js')
    .concat('src/**/*.spec.js')
    .value();
